...
def generate_label_json_PL(doc, items, service_type="Peneus Hub", label_type=None, custom_header=False, skip_custom_printers=False,submission_date=""):
    """
    ...
    # Initialize the result structure
        customer = frappe.get_doc("Customer", doc.custom_pl_customer)
    ...
    mr_transporter = mr.custom_transporter_name
    mr_shipping_to = mr.custom_shipping_to
    mr_sp_address = frappe.get_doc("Address", mr.custom_shipping_address_name)
    address_line = mr_sp_address.address_line1
    city = mr_sp_address.city
    state = mr_sp_address.state

    # for item in items:

        # Get accepted and rejected bundles
        # accepted_bundles = (item.get('serial_and_batch_bundle') or '').split(',')
        ...
        #         continue
            ...
            # Get bundle document
    bundle_doc = frappe.get_doc('Pick List', doc.name)

            # Process each entry in the bundle
    for index, entry in enumerate(doc.custom_item_locations, 1):
        ...
        if service_type == "Peneus Hub":
            ...
            label_data = {
                ...
                "item_code": serial_doc.item_code or "",
                "barcode": serial_doc.custom_barcode or "",
                "shipping_to": mr_shipping_to,
                "transporter_name": mr_transporter,
                ...
                "product": {
                    "id": serial_doc.name,
                    "slug": custom_slug,
                    "code": serial_doc.item_code or "None",
                    "description": serial_doc.item_name,
                    "sku": serial_doc.custom_upc or "None"

                },
                "packaging": {
                    "index": index,
                    "total": len(bundle_doc.custom_item_locations)
                },
...
def download_label_json_PL(doc, items=None, service_type="Peneus Hub", label_type=None, custom_header=False, skip_custom_printers=False,submission_date=""):
    """
    ...
    label_data = generate_label_json_PL(doc, items, service_type, label_type, custom_header, skip_custom_printers,submission_date)
...
@frappe.whitelist()
def generate_json_labels_PL(doctype, docname, service_type="Peneus Hub", label_type=None, custom_header=False, skip_custom_printers=False,customer_has_own_printer=False,submission_date=""):
    """
    ...
    try:
        ...
        items = doc.get("custom_item_locations")
        # if service_type == "Peneus Hub":
        ...
        return download_label_json_PL(doc, items, service_type, label_type, custom_header, skip_custom_printers,submission_date)
